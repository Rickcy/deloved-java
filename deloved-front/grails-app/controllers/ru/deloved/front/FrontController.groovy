...
@Secured(["permitAll"])
class FrontController  extends FilteredController {
	@Override
...
//				order('account.' + filter.sort ?: "account.name", filter.order ?: "asc")
	...
	def index(FrontFilter indexFilter) {
		...
		//def mainGoods = Item.findAllByShowMainAndCategoryTypeAndPhotoIsNotNull(true, CategoryType.findByCode('GOOD'), [max: 20])
