...
<div class="row minicart lc">
	<div class="col-xs-4 col-sm-3" style=" text-align: center;">
		...
		<g:else>
			<img class="img-thumbnail img_left" src="${resource(dir: 'images', file: 'front/goods.png')}" style="border: none"/>
		</g:else>
	...
	<g:link controller="goods"  action="item" id="${itemInstance.id}"><g:fieldValue bean="${itemInstance}" field="name"/></g:link>
		...
		<div class="date_create"><g:formatDate date="${itemInstance?.dateCreated}" format=" dd.MM.yyyy HH:mm:ss"/></div>
			...
			<g:link controller="goods" class="podr" style="text-align: center" action="item" id="${itemInstance.id}">
				Подробнее
			</g:link>
	...
	<div class="col-xs-12 col-sm-5" >
		...
		<div class="gray_block">
			...
			<g:if test="${freeAccount && isMyAccount}">
				<button class="deal_button" style="border: none;min-width: 143px;background-color: grey; text-align: left" disabled="true">Предложить сделку</button>
			</g:if>
			...
			<g:else>
				<sec:ifNotLoggedIn>
					<g:link class="deal_button"  controller="login" action="auth"
							...
							data-target="#myModal">Предложить сделку</g:link>	</sec:ifNotLoggedIn>
				<sec:ifLoggedIn>
