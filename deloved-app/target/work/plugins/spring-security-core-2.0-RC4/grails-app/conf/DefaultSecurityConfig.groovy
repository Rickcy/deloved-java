...
security {
	...
	failureHandler {
		defaultFailureUrl = uri+'/login/authfail?login_error=1'
		ajaxAuthFailUrl = uri+'/login/authfail?ajax=true'
		exceptionMappings = [:]
		useForward = false
		allowSessionCreation = true
	}
	...
	switchUser {
		switchUserUrl = uri+'/j_spring_security_switch_user'
		exitUserUrl = uri+'/j_spring_security_exit_user'
