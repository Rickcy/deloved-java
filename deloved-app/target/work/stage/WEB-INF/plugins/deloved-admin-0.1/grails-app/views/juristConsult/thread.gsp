<%@ page import="ru.deloved.recall.JuristConsult" %>
<%@ page import="ru.deloved.recall.JuristConsultStatus" %>
<
<html>
<head>
    ...
    <div id="ticket-thread" class="content scaffold-list" role="main">
        ...
        <div class="lead">
            <sec:ifAnyGranted roles="ROLE_ACCOUNT">
                <g:if test="${juristConsultInstance.jurist!=null}">
                    <label for="account">Консультация с :<a id="judge" href="javascript:void(0)">${juristConsultInstance.jurist.fio}</a></label>
                        ...
                        <div class="modal-dialog" align="center">
                            ...
                            <div class="modal-content" style="width: 120%">
                                ...
                                <div class="modal-header" >
                                    ...
                                    <h4 class="modal-title" style="text-align: center">${juristConsultInstance.jurist.fio}</h4>
                                </div>
                                ...
                                <div class="modal-body" style="font-size: 14px!important;">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="value-col">
                                            <div name="avatar">
                                                <g:if test="${juristConsultInstance.jurist.avatarThumb}">
                                                    <img
                                                            src="${createLink(controller: 'profile', action: 'avatar', id: juristConsultInstance.jurist.id, params: [name: juristConsultInstance.jurist.avatarThumb?.file])}"/>
                                                </g:if>
                                                <g:else>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="value-col ft">
                                            <input id="city" name="city" readonly value="${juristConsultInstance.jurist?.city?.name}" data-old="${juristConsultInstance.jurist?.city?.name}"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="value-col ft">
                                            <input id="cellPhone" name="cellPhone" readonly value="${juristConsultInstance.jurist.dayOfWork}" data-old="${juristConsultInstance.jurist.dayOfWork}"
                                                   placeholder="Отсутствует"/>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="value-col ft">
                                            <p id="dateCreated" name="dateCreated"><g:formatDate date="${juristConsultInstance.jurist.dateCreated}" format="dd.MM.yyyy"/></p>
                                        </div>
            ...
            <sec:ifAnyGranted roles="ROLE_JURIST,ROLE_ADMIN">
                ...
                <span id="account">${juristConsultInstance.account.name}</span>
            </sec:ifAnyGranted>
        </div>

        <div>
            <label for="status">Статус:</label>
            <span id="status">${message(code: 'consult.status.' + juristConsultInstance.status(), default: juristConsultInstance.status())}</span>
        </div>
        ...
        <sec:ifAnyGranted roles="ROLE_ADMIN,ROLE_JURIST">
                ...
                background-color: #f4f4f4;box-shadow: inset 0px 0px 10px #d7d7d7">
                    ...
                    <script type="application/javascript">
                        function setStatus(status) {
                            if (confirm('${message(code: 'deal.button.status.confirm.message')}')) {
                    ...
                    <g:form id="statusForm" url="[resource: juristConsultInstance, action: 'post']">
                        <g:hiddenField name="version" value="${juristConsultInstance?.version}"/>
                ...
                <sec:ifAnyGranted roles="ROLE_ACCOUNT">
                    ...
                    <script type="application/javascript">
                        var myDropzone = new Dropzone("button#sendFiles", {
                            method: 'POST',
                            url: '${createLink(controller: 'juristConsult', action: 'upload', id: juristConsultInstance.id)}',
                            clickable: true,
                            parallelUploads: 8,
                            uploadMultiple: true,
    ...
    <script type="application/javascript">
        ...
        $(document).ready(function(){
            ...
            _node.onsubmit = function(event) {
                ...
                $.ajax({
                    ...
                    url: '${createLink(resource: juristConsultInstance, action: 'post')}',
                    success:function(data,textStatus){
                        pullMessages();
                    },
                    error:function(XMLHttpRequest,textStatus,errorThrown){
            ...
            _node.onsubmit = function(event){
                ...
                $.ajax({
                    type: 'POST',
                    data: jQuery(this).serialize(),
                    url: '${createLink(controller: 'juristConsult', action: 'post', resource: juristConsultInstance)}',
                    success:function(data,textStatus){
                        $('#messageText').val('');
                        pullMessages();
                    },
        ...
        function getNewPosts() {
            ...
            if (postList.length) {
                ...
                jQuery.ajax({
                    type:'GET',
                    url:'${createLink([controller: 'juristConsult', action: 'getLatestPosts'])}/'+lastPost.getAttribute('value'),
                    success:function(data,textStatus){
                        if (data != 'NO_CONTENT') {
        ...
        var postTimer = ${juristConsultInstance.status};
        pullMessages();
        displayCache = {}
