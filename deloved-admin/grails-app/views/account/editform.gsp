<div class="modal-header">
	...
	<h4 class="modal-title" id="myModalLabel"><g:message code="account.edit.label"/></h4>
</div>
...
<div class="modal-body">
	...
	<g:hasErrors bean="${objInstance}">
		<ul class="alert alert-danger errors" role="alert">
			...
			<g:if test="${objInstance?.hasProperty('affiliates')}">
				<g:each in="${objInstance.affiliates}" var="affInstance" status="i">
					<g:eachError bean="${affInstance}" var="error">
						<li <g:if test="${error in org.springframework.validation.FieldError}">data-field-id="${error.field}"</g:if>><g:message error="${error}"/></li>
					</g:eachError>
				</g:each>
			</g:if>
