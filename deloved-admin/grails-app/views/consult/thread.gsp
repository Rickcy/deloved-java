<%@ page import="ru.deloved.Consult" %>
<%@ page import="ru.deloved.ConsultStatus" %>
...
<g:else>
	...
	<div id="consult-thread" class="content scaffold-list" role="main">
		...
		<g:link class="btn btn-default" action="index"><span class="glyphicon glyphicon-chevron-left"></span> <g:message
				code="consult.backallconsults"/></g:link>

		<div class="lead">
			<sec:ifAnyGranted roles="ROLE_ACCOUNT">
				<g:if test="${consultInstance.jurist}">
					<label for="account">Консультация с :<a id="judge" href="javascript:void(0)">${consultInstance.jurist.fio}</a></label>
						...
						<div class="modal-dialog" align="center">
							...
							<div class="modal-content" style="width: 120%">
								...
								<div class="modal-header" >
									...
									<h4 class="modal-title" style="text-align: center">${consultInstance.jurist.fio}</h4>
								</div>
								...
								<div class="modal-body" style="font-size: 14px!important;">
									...
									<div class="row">
										...
										<div class="value-col">
											<div name="avatar">
												<g:if test="${consultInstance.jurist.avatarThumb}">
													<img
															src="${createLink(controller: 'profile', action: 'avatar', id: consultInstance.jurist.id, params: [name: consultInstance.jurist.avatarThumb?.file])}"/>
												</g:if>
												<g:else>
									...
									<div class="row">
										...
										<div class="value-col ft">
											<input id="city" name="city" readonly value="${consultInstance.jurist?.city?.name}" data-old="${consultInstance.jurist?.city?.name}"
									...
									<div class="row">
										...
										<div class="value-col ft">
											<input id="cellPhone" name="cellPhone" readonly value="${consultInstance.jurist.cellPhone}" data-old="${consultInstance.jurist.cellPhone}"
												   placeholder="Отсутствует"/>
									...
									<div class="row">
										...
										<div class="value-col ft">
											<p id="dateCreated" name="dateCreated"><g:formatDate date="${consultInstance.jurist.dateCreated}" format="dd.MM.yyyy"/></p>
										</div>
			...
			<sec:ifAnyGranted roles="ROLE_JURIST,ROLE_ADMIN">
				...
				<span id="account">${consultInstance.account.name}</span>
			</sec:ifAnyGranted>
		</div>
		...
		<div>
			...
			<span id="status">${message(code: 'consult.status.' + consultInstance.status(), default: consultInstance.status())}</span>
		</div>

		<g:if test="${[ConsultCategory.Dispute.value(), ConsultCategory.Claim.value()].contains(consultInstance.consultcat)}">
			<label for="consultDeal">Рассмотрение возможности ${consultInstance.consultcat == ConsultCategory.Dispute.value() ? 'процедуры медиации' : 'проведения третейского'} по сделке:
			</label>
			<span id="consultDeal"><g:link controller="deal" action="thread" id="${consultInstance.deal.id}">${consultInstance.deal.id}</g:link></span>
			<br>
			<label for="claimant">Первая сторона:</label>
			<span id="claimant"><g:link url="[controller: 'company', id: consultInstance.account.id]">${consultInstance.account.name}</g:link></span>

			<label for="defendant">Вторая сторона:</label>
			<span id="defendant"><g:link url="[controller: 'company', id: consultInstance.defendant?.id]">${consultInstance.defendant?.name}</g:link></span>

			<sec:ifAllGranted roles="ROLE_ADMIN,ROLE_JURIST,ROLE_MANAGER">
				<div class="row">
					<div class="col-md-10 col-md-offset-1">
						<g:form id="subjectForm" name="subjectForm" controller="${consultInstance.consultcat == ConsultCategory.Dispute ? 'dispute' : 'claim'}" action="save" target="_blank">
							<input type="hidden" readonly name="account" value="${consultInstance.account.id}">
							<input type="hidden" readonly name="partner" value="${consultInstance.defendant?.id}">
							<input type="hidden" readonly value="${consultInstance.deal.id}">
							<label for="subjectArea">Суть претенезии:</label>
							<textarea required rows="10" id="subjectArea" class="form-control" style="resize: none; margin-bottom: 10px" name="subject"></textarea>
							<div align="right"><input form="subjectForm" class="btn btn-warning" type="submit" value="Открыть иск/спор"></div>
						</g:form>
					</div>
				</div>
			</sec:ifAllGranted>
		...
		<hr>
		<sec:ifAnyGranted roles="ROLE_ACCOUNT,ROLE_JURIST">
		...
		<sec:ifAnyGranted roles="ROLE_ADMIN">
			...
			<g:if test="${statusList}">
				<div class="row" style="margin-top: 10px">
					...
					background-color: #f4f4f4;box-shadow: inset 0px 0px 10px #d7d7d7">
						...
						<script type="application/javascript">
							function setStatus(status) {
								if (confirm('${message(code: 'consult.button.status.confirm.message')}')) {
						...
						<div class="btn-group">
							...
							<ul class="dropdown-menu" role="menu">
								<g:each in="${statusList}" var="st">
									<li><a href="#"
										   onclick="return setStatus(${st.value()});">${message(code: 'consult.status.' + st, default: st)}</a>
									</li>
								</g:each>
							</ul>
						</div>
					...
					<g:form id="statusForm" url="[resource: consultInstance, action: 'post']">
						<g:hiddenField name="version" value="${consultInstance?.version}"/>
		...
		<g:if test="${canPost}">
			...
			<div style="padding: 0;margin-bottom: 5%;" align="right">
				<button  id="messageSubmit" class="btn btn-success" name="messageSubmit" type="submit" form="messageForm" style="margin-bottom: 10px;border-radius: 15px; float: left;width: 220px;">
					...
					${message(code: 'consult.send.label')}
				</button>
				<sec:ifAnyGranted roles="ROLE_ACCOUNT">
					...
					<script type="application/javascript">
						var myDropzone = new Dropzone("button#sendFiles", {
							method: 'POST',
							url: '${createLink(controller: 'consult', action: 'upload', id: consultInstance.id)}',
							clickable: true,
							parallelUploads: 8,
							uploadMultiple: true,
	...
	<script type="application/javascript">
		...
		$(document).ready(function(){
			...
			_node.onsubmit = function(event) {
				...
				$.ajax({
					...
					url: '${createLink(resource: consultInstance, action: 'post')}',
					success:function(data,textStatus){
						pullMessages();
					},
					error:function(XMLHttpRequest,textStatus,errorThrown){
			...
			_node.onsubmit = function(event){
				...
				$.ajax({
					type: 'POST',
					data: jQuery(this).serialize(),
					url: '${createLink(controller: 'consult', action: 'post', resource: consultInstance)}',
					success:function(data,textStatus){
						$('#messageText').val('');
						pullMessages();
					},
		...
		function getNewPosts() {
			...
			if (postList.length) {
				...
				jQuery.ajax({
					type:'GET',
					url:'${createLink([controller: 'consult', action: 'getLatestPosts'])}/'+lastPost.getAttribute('value'),
					success:function(data,textStatus){
						if (data != 'NO_CONTENT') {
		...
		var postTimer = ${consultInstance.status};
		pullMessages();
		displayCache = {}
